  ...
  class ProcessBatchRequest
    ...
    def initialize(attributes = {})
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CityPayApiClient::ProcessBatchRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def client_account_id=(client_account_id)
      ...
      if client_account_id.to_s.length < 3
        fail ArgumentError, 'invalid value for "client_account_id", the character length must be greater than or equal to 3.'
      end
