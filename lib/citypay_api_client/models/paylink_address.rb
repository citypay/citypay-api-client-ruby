  ...
  class PaylinkAddress
    ...
    def initialize(attributes = {})
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CityPayApiClient::PaylinkAddress`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@label.nil? && @label.to_s.length < 2
        invalid_properties.push('invalid value for "label", the character length must be greater than or equal to 2.')
      end
    ...
    def label=(label)
      ...
      if label.to_s.length < 2
        fail ArgumentError, 'invalid value for "label", the character length must be greater than or equal to 2.'
      end
