...
module CityPayApiClient
  class PaylinkTokenCreated
    ...
    def initialize(attributes = {})
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CityPayApiClient::PaylinkTokenCreated`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [attachments, bps, date_created, errors, id, identifier, merchantid, mode, qrcode, result, server_version, source, token, url, usc].hash
    end
