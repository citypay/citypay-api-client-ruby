  ...
  class ExternalMPI
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CityPayApiClient::ExternalMPI` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CityPayApiClient::ExternalMPI`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@cavv.nil? && @cavv.to_s.length > 20
        invalid_properties.push('invalid value for "cavv", the character length must be smaller than or equal to 20.')
      end

      if !@enrolled.nil? && @enrolled.to_s.length > 1
        invalid_properties.push('invalid value for "enrolled", the character length must be smaller than or equal to 1.')
      end

      if !@xid.nil? && @xid.to_s.length > 20
        invalid_properties.push('invalid value for "xid", the character length must be smaller than or equal to 20.')
      end
    ...
    def hash
      [authen_result, cavv, eci, enrolled, xid].hash
    end
