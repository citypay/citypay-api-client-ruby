  ...
  class VerificationRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CityPayApiClient::VerificationRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CityPayApiClient::VerificationRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'cp_card_token')
        self.cp_card_token = attributes[:'cp_card_token']
      end
      ...
      if attributes.key?(:'threeds_token')
        self.threeds_token = attributes[:'threeds_token']
      end
    ...
    def cardnumber=(cardnumber)
      ...
      if cardnumber.to_s.length < 12
        fail ArgumentError, 'invalid value for "cardnumber", the character length must be greater than or equal to 12.'
      end
    ...
    def csc=(csc)
      ...
      if csc.to_s.length < 3
        fail ArgumentError, 'invalid value for "csc", the character length must be greater than or equal to 3.'
      end
    ...
    def uuid=(uuid)
      ...
      if uuid.to_s.length < 36
        fail ArgumentError, 'invalid value for "uuid", the character length must be greater than or equal to 36.'
      end
