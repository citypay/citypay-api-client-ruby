...
module CityPayApiClient
  class AirlineAdvice
    ...
    def initialize(attributes = {})
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CityPayApiClient::AirlineAdvice`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @transaction_type.to_s.length < 3
        invalid_properties.push('invalid value for "transaction_type", the character length must be greater than or equal to 3.')
      end
    ...
    def transaction_type=(transaction_type)
      ...
      if transaction_type.to_s.length < 3
        fail ArgumentError, 'invalid value for "transaction_type", the character length must be greater than or equal to 3.'
      end
