  ...
  class BatchTransaction
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CityPayApiClient::BatchTransaction` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CityPayApiClient::BatchTransaction`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @account_id.to_s.length < 5
        invalid_properties.push('invalid value for "account_id", the character length must be greater than or equal to 5.')
      end
    ...
    def account_id=(account_id)
      ...
      if account_id.to_s.length < 5
        fail ArgumentError, 'invalid value for "account_id", the character length must be greater than or equal to 5.'
      end
    ...
    def hash
      [account_id, amount, identifier, merchantid].hash
    end
