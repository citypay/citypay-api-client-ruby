  ...
  class Error
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CityPayApiClient::Error` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CityPayApiClient::Error`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@code.nil? && @code.to_s.length < 3
        invalid_properties.push('invalid value for "code", the character length must be greater than or equal to 3.')
      end
    ...
    def code=(code)
      ...
      if code.to_s.length < 3
        fail ArgumentError, 'invalid value for "code", the character length must be greater than or equal to 3.'
      end
