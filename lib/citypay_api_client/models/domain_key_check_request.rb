  ...
  class DomainKeyCheckRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CityPayApiClient::DomainKeyCheckRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CityPayApiClient::DomainKeyCheckRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @domain_key.to_s.length < 64
        invalid_properties.push('invalid value for "domain_key", the character length must be greater than or equal to 64.')
      end
    ...
    def domain_key=(domain_key)
      ...
      if domain_key.to_s.length < 64
        fail ArgumentError, 'invalid value for "domain_key", the character length must be greater than or equal to 64.'
      end
