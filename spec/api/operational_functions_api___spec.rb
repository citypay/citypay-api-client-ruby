...
describe 'OperationalFunctionsApi' do
  ...
  describe 'acl_check_request test' do
    it 'should work' do
      response = @api_instance.acl_check_request(CityPayApiClient::AclCheckRequest.new(ip: "1.1.1.1"))
      expect(response.ip).to eq("1.1.1.1")
    end
  end

  describe 'domain_key_check_request test' do
    it 'should work' do
      key = @api_instance.domain_key_gen_request(CityPayApiClient::DomainKeyRequest.new(domain: ["test.citypay.com"], merchantid: merchant_id))
      response = @api_instance.domain_key_check_request(key)
      expect(response.domain[0]).to eq('test.citypay.com')
    end
  end

  describe 'domain_key_gen_request test' do
    it 'should work' do
      response = @api_instance.domain_key_gen_request(CityPayApiClient::DomainKeyRequest.new(domain: ["test.citypay.com"], merchantid: merchant_id))
      expect(response.domain_key).to_not be_empty
    end
  end

  describe 'list_merchants_request test' do
    it 'should work' do
      response = @api_instance.list_merchants_request(client_id)
      expect(response.merchants).not_to be_empty
    end
  end

  describe 'ping_request test' do
    it 'should work' do
      response = @api_instance.ping_request(identifier: "test")
      expect(response.code).to eq("044")
    end
  end
